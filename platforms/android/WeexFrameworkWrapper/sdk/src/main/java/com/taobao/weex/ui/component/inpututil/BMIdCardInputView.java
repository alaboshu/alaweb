...
public class BMIdCardInputView extends KeyboardView {
    private Context mContext;
    private Keyboard mKeyBoard;
    private String mReturnKeyType;

    public BMIdCardInputView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
    }

    public BMIdCardInputView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void drawText(Canvas canvas, Keyboard.Key key) {
        ...
        if (key.label != null) {
            ...
            key.icon.setBounds(key.x + (key.width - key.icon.getIntrinsicWidth()) / 2, key.y +
                            (key.height - key.icon.getIntrinsicHeight()) / 2,
                    key.x + (key.width - key.icon.getIntrinsicWidth()) / 2 + key.icon
                            .getIntrinsicWidth(), key.y + (key.height - key.icon
                            .getIntrinsicHeight()) / 2 + key.icon.getIntrinsicHeight());
    ...
    public void setReturnType(String returnType) {
        mReturnKeyType = returnType;
    }

    public void getReturnkeyString(Keyboard.Key key){
        if(key==null){
            return;
        }
