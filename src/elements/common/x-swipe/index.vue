<template>
  <div class="x-swipe" @touchstart="onTouchstart" @touchmove="onTouchmove" @touchend="onTouchend" @touchcancel="onTouchend">
    <div :style="wrapperStyle" class="x-swipe__wrapper" @transitionend="$emit('change', activeIndicator)">
      <slot/>
    </div>
    <div class="x-swipe__indicators" v-if="showIndicators && count > 1">
      <i v-for="index in count" :key="index" :class="{ 'x-swipe__indicator--active': index - 1 === activeIndicator }" />
    </div>
  </div>
</template>
