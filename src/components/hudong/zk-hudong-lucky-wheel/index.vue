<template>
  <div class="container">
      ...
      <div class="wheel-main">
        ...
        <div class="wheel-bg" :style="{transform:rotate_angle,transition:rotate_transition}">
          <div class="prize-list">
            <div class="prize-item" v-for="(item,index) in widgetModel" :key="index">
              <div class="prize-pic">
                <img :src="item.img">
              </div>
    ...
    <div class="main">
      ...
      <div class="bg-p"></div>
      <div class="content">
        <div class="lottery_ticket">今日免费抽奖次数： {{ getAwardCount}}</div>
      </div>
      <div class="tip">
    ...
    <div class="hiddenMask" v-show="hiddenMak"></div>
  </div>
</template>
<script>
  export default {
    ...
    computed: {
      toast_title () {
        return this.hasPrize
          ? '恭喜您，获得' + this.awardText + ' ' + this.widgetModel[this.index].grade
          : '未中奖'
      },
      toast_pictrue () {
        return this.hasPrize
    ...
    methods: {
      async  init () {
        ...
        var res = await this.$api.httpGet('/Api/Hudong/GetAwards', para)
        if (res.status === 1) {
          this.Status = res.status
          this.widgetModel = res.result.item2.awards
          this.drawCount = res.result.item2.drawCount
          this.awardId = res.result.item2.id
          this.getAwardCount = res.result.item2.drawCount
          this.drawRule = res.result.item2.drawRule
        } else {
          this.Status = res.status
      ...
      async rotate_handle () {
        if (!this.$user.isLogin()) {
          ...
          if (this.getAwardCount === 0) {
            this.$api.toastWarn('您当前剩余抽奖次数为0')
          } else if (this.Status !== 1) {
            this.$api.toastWarn(this.ThisStatus)
          } else {
            var para = {
              id: this.awardId,
              key: 'BigWheel',
            ...
            var Ares = await this.$api.httpGet('/Api/Hudong/draw', para)
            if (Ares.status === 1) {
              this.awardCount = Ares.result.returnObject
              this.getAwardCount--
              for (var i = 0; i < this.widgetModel.length; i++) {
                if (this.widgetModel[i].awardId === this.awardCount) {
                  this.index = i
                }
              }
              this.rotating()
      ...
      rotating (awardId) {
        ...
        if (type === 0) {
          var rotateAngle = this.start_rotating_degree + randCircle * 360 + resultAngle[resultIndex] -
      ...
      game_over () {
        ...
        this.hasPrize = this.widgetModel[this.index].grade
        if (this.hasPrize !== null) {
          this.hasPrize = true
        } else {
          this.hasPrize = false
        }
